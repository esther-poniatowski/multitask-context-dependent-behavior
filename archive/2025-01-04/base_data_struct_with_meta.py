...
class DataStructure(Generic[T], metaclass=DataStructMeta):
    ...
    # Placeholders - Automatically set by the metaclass `DataStructMeta`
    dims: Tuple[str, ...] = ()
    coords: Tuple[str, ...] = ()
    ...
    def __init__(self, data: Optional[npt.NDArray], **coords) -> None:
        """
        ...
        See Also
        --------
        :meth:`Coordinate.empty`
            Create an empty coordinate of the expected type. Assumed to be implemented in the
            coordinate classes.
        """
        # Initialize flags to indicate empty state
    ...
    def set_data(self, data: npt.NDArray) -> None:
        ...
        self._has_data = True  # indicate that `data` has content
    ...
    def set_coords(self, **coords) -> None:
        ...
        Warning
        -------
        The attribute :attr:`n` has to been set beforehand by the method :meth:`set_dims`.
        ...
        """
        for coord_name, tpe in self.coord2type.items():
            # Check coordinate presence
            if coord_name not in coords:
                raise ValueError(f"Missing coordinate: {coord_name}")
            # Check coordinate type
            if not isinstance(coords[coord_name], tpe):
                raise TypeError(f"Invalid type for '{coord_name}': {type(coords[coord_name])}")
            # Check coordinate length
            if len(coords[coord_name]) != self.n[self.coord2dim[coord_name]]:
                raise ValueError(f"Invalid length for '{coord_name}': {len(coords[coord_name])}")
            setattr(self, coord_name, coords[coord_name])
        self._has_coords = True  # indicate that coordinate attributes have content
