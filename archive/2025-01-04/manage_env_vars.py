...
Arguments
---------
--method : str, among {'set_all', 'unset', 'unset_all', 'check'}
    Method to execute.
...
class EnvVarManager:
    ...
    def unset_all(self):
        """Unset all the environment variables set in the Conda environment."""
        # Get all environment variables currently set
        output = subprocess.run("conda env config vars list", shell=True, capture_output=True, text=True)
        env_vars = output.stdout.strip().split('\n') # split by line
        # Extract variable names
        var_names = [line.split('=')[0].strip() for line in env_vars if '=' in line]
        # Unset each variable
    ...
    def _apply_method(self, method: Callable):
        ...
        if self.args.method == 'set_all':
            ...
            if not self.args.vars:
                raise ValueError(f"Variable name(s) required for method: {self.args.method}")
            else:
