...
from core.processors.preprocess.firing_rates import extract_trial
from core.processors.preprocess.firing_rates import slice_epoch
from core.processors.preprocess.firing_rates import join_epochs
from core.processors.preprocess.firing_rates import spikes_to_rates
from core.processors.preprocess.firing_rates import smooth
...
@pytest.mark.parametrize(
    ...
    argvalues=[(spikes, trials, expected), (np.array([]), np.array([]), np.array([]))],
    ids=["with_content", "emtpy"],
)
def test_extract_trial(spikes, trials, expected):
    """
    Tests for :func:`core.processors.preprocess.firing_rates.extract_trial`.

    Parametrized for the argument ``data``.

    Test Inputs
    -----------
    trials [with_content]: :obj:`core.types.NumpyArray` of int
        3 trials.
    spikes [with_content]: :obj:`core.types.NumpyArray` of float
        2 spikes per trial.
    trials, spikes [empty]: :obj:`core.types.NumpyArray`
...
@pytest.mark.parametrize(
    "spk, expected",
    argvalues=[(spk, expected), (np.array([]), np.array([]))],
    ids=["with_content", "empty"],
)
def test_slice_epoch(spk, expected):
    """
    Test for :func:`core.processors.preprocess.firing_rates.slice_epoch`.
    ...
    Expected Outputs
    ----------------
    expected [with_content]: :obj:`core.types.NumpyArray`
        ``[0.0, 0.1, 0.2]``
...
def test_join_epochs(spk, expected):
    """
    Test for :func:`core.processors.preprocess.firing_rates.join_epochs`.
...
@pytest.mark.parametrize(
    "spk, expected",
    argvalues=[(spk, expected), (np.array([]), expected_empty)],
    ids=["with_content", "empty"],
)
def test_spikes_to_rates(spk, expected):
    """
    Test for :func:`core.processors.preprocess.firing_rates.spikes_to_rates`.
...
def test_smooth():
    """
    Test for :func:`core.processors.preprocess.firing_rates.smooth`.
    ...
    Expected Outputs
    ----------------
    expected: :obj:`core.types.NumpyArray`
